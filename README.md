...
| 구분 | 경로 | 설명 |
|---|---|---|
| ROS 2 노드 (Python) | [`src/`](./src/) | 키패드·프로필 계산·시트/벨트 제어·아두이노 브릿지 5개 노드 |
| 아두이노 스케치 (C++) | [`arduino/smart_seat_mega/`](./arduino/smart_seat_mega/) | 리니어 액추에이터 5개 구동 + HC-SR04 초음파 측정 |
| 커스텀 메시지 | [`msg/TargetSeatProfile.msg`](./msg/TargetSeatProfile.msg) | 벨트 높이 + 액추에이터 목표 배열 |
| 실행 파일 | [`launch/smart_seat.launch.py`](./launch/smart_seat.launch.py) | 노드 5개 일괄 실행 |
| 데이터 | [`profiles.json`](./profiles.json), [`belt_home.json`](./belt_home.json) | 가족 프로필 · 다이나믹셀 홈 위치 |

> 데이터 흐름: `keypad_node → /body_data → profile_inference_node → /target_seat_profile → seat_controller_node / belt_controller_node → /motor_cmd → arduino_bridge_node`
> 초음파는 아두이노가 측정 → `arduino_bridge_node`가 `/ultrasonic_distance`로 중계 → `keypad_node` 수신.
