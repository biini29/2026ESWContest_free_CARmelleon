...
등받이 내부에는 총 5개의 리니어 액추에이터가 일정 간격으로 수직 배치되어 등받이 형상을 능동적으로 변형시킵니다. 액추에이터는 전용 고정 마운트 플레이트(`linear_motor_mount_plate`)에 구멍을 뚫어 나사(Screw) 체결 방식으로 고정하였으며, 백레스트 메인 외각 프레임(`backrest_outer_frame`)에 안정적으로 장착됩니다.

외부 표면은 착좌감과 실제 시트 질감을 구현하기 위해 프레임 및 액추에이터 구동부를 **천과 솜 커버**로 감싸 마감하였습니다.

- **액추에이터 결합 방식:** `linear_motor_mount_plate` 부품에 가공된 홀을 통해 리니어 액추에이터를 나사 고정
- **치수 및 간격 구현:** 
  - 하단 엉덩이 시트(시트 쿠션) 높이: **75mm**
  - 액추에이터 수직 위치: 시트 면 상단 기준 첫 번째 **160mm** 위치부터 **85mm 간격**으로 상단까지 5개 연속 배치 (160mm, 245mm, 330mm, 415mm, 500mm)
  - 각각의 액추에이터가 구동부 쉘(`backrest_shell`)을 인출/인입하여 세밀한 요추 지지 곡면 형성
- **연결 폴더:** [`Linear Moter Lumber Structure/`](./Linear%20Moter%20Lumber%20Structure/) 내부 자료 참고

| 항목 | 값 |
|---|---|
| 프레임 재질 | 3D 프린팅 (PLA/PETG) 및 메인 구조 프레임 |
| 액추에이터 고정 방식 | 마운트 플레이트(`linear_motor_mount_plate`) 타공 후 나사 체결 고정 |
| 등받이 곡면 재질 | 커스텀 프린팅 쉘 패널 (`backrest_shell`) + 천 & 솜 완충 커버 |
