...
등받이 내부에는 총 5개의 리니어 액추에이터가 일정 간격으로 수직 배치되어 등받이 형상을 능동적으로 변형시킵니다. 액추에이터는 전용 고정 마운트 플레이트(`linear_motor_mount_plate`)에 구멍을 뚫어 나사로 직접 결합·고정하였으며, 백레스트 메인 외각 프레임(`backrest_outer_frame`)에 안정적으로 장착됩니다.
